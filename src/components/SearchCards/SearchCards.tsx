...
import { useFetchCardsSearch } from "@/hooks/useFetchCardsSearch";
...
const SearchCards = ({ slug }: SearchCardsProps) => {
  const [page, setPages] = useState(1);


  const {cards,totalPage} = useFetchCardsSearch({ page, slug });
    ...
    <div>
      <div>
        ...
        <div className="w-full mb-4 flex items-center justify-center gap-4 ">
          {page > 1 && (
            <button onClick={() => setPages((r) => r - 1)} className="w-8 ">
              <svg
